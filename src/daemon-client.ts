...
export const daemonClient = {
  ping: () => request<{ok: boolean; pid: number; protocolVersion: number}>('ping'),
  listSessions: (includeArchived = false) => request<SessionSummary[]>('sessions.list', includeArchived ? {includeArchived: true} : {}),
  ...
  checkpointRun: (runId: string) => request<Run['checkpoint']>('runs.checkpoint', {runId}),
  listRecipes: (directory: string) => request<RecipeDefinition[]>('recipes.list', {directory}),
  recipeReceipts: (directory?: string) => request<RecipeReceipt[]>('recipes.receipts', directory ? {directory} : {}),
  ...
  archiveSession: (sessionId: string) => request<SessionSummary>('sessions.archive', {sessionId}),
  restoreSession: (sessionId: string) => request<SessionSummary>('sessions.restore', {sessionId}),
  deleteSession: (sessionId: string, approvalId: string) => request<{deleted: true; sessionId: string}>('sessions.delete', {sessionId, approvalId}),
  ...
  setMasterBrief: (project: string, brief: string) => request<import('./daemon-protocol.js').CoordinationState>('coordination.brief.set', {project, brief}),
  createTask: (project: string, task: {title: string; description?: string; role?: string; provider?: ProviderId; source?: 'manual' | 'spec' | 'planner'; sessionId?: string; designHandoff?: import('./daemon-protocol.js').DesignHandoffSpec; dependsOn?: string[]}) => request<import('./daemon-protocol.js').CoordinationState>('coordination.task.create', {project, ...task}),
  assignTask: (project: string, taskId: string, assignment: {sessionId?: string; provider?: ProviderId; role?: string}) => request<import('./daemon-protocol.js').CoordinationState>('coordination.task.assign', {project, taskId, ...assignment}),
  setTaskDependencies: (project: string, taskId: string, dependsOn: string[]) => request<import('./daemon-protocol.js').CoordinationState>('coordination.task.dependencies.set', {project, taskId, dependsOn}),
  ...
  upsertAccount: (params: {provider: ProviderId; id: string; mode: CredentialMode; label: string; apiKey?: string; baseUrl?: string; model?: string; sameIdentityAs?: string}) => request<CredentialChainState>('credentials.upsertAccount', params),
