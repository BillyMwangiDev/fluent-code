...
async function waitFor(condition: () => boolean | Promise<boolean>, timeoutMs = 4_000) {
...
const server = net.createServer(socket => socket.on('data', () => socket.end(JSON.stringify({id: 'probe', ok: true, result: {protocolVersion}}) + '\\n')));
server.listen(localSocket);
const exitOnceFile = process.env.FLUENT_TEST_EXIT_ONCE_FILE;
if (exitOnceFile && !require('node:fs').existsSync(exitOnceFile)) {
  require('node:fs').writeFileSync(exitOnceFile, 'started');
