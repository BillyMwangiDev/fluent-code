//! Standalone per-process resource sampler, spoken to over stdin/stdout newline-JSON.
//!
//! Forked from T3 Code's native resource monitor (github.com/pingdotgg/t3code,
//! native/resource-monitor/src/main.rs, MIT licensed) — the protocol and collection logic
//! below are theirs almost verbatim; only the package name changed. Ported because a crash
//! here must never take fluentd down with it, and `sysinfo` inside the Node process would
//! risk exactly that. fluentd (not this binary) decides what "root" and "external" processes
//! mean — see src/resource-monitor-client.ts.
